...
class MainService(name: String = "service"): IntentService(name) {
    ...
    private fun loadNowPlaying(language: String) {
        Thread {
            ...
            broadcastIntent.putParcelableArrayListExtra(MAIN_MOVIE_NOW_LIST_EXTRA, nowPlayingDTO.results)
    ...
    private fun loadUpcoming(language: String) {
        Thread {
            ...
            broadcastIntent.putParcelableArrayListExtra(MAIN_MOVIE_UPCOMING_LIST_EXTRA, upcomingDTO.results)
            LocalBroadcastManager.getInstance(this).sendBroadcast(broadcastIntent)
        }.start()
    }
